...
class Proflow(nn.Module):
    ...
    def sample_binder_bylength_reference(self,Target=0,design_num=1,Length=30,fixed_com_idx=[],hotspot=None):
        ...
        target: design target, the chain need to be redesign  comidx

        '''

        model_name=self._inf_cfg.ckpt_path.split('/')[-3]

        design_class_name='7rby'
        design_path='/home/junyu/project/binder_target/GP120/2ny3_b/preprocessed/design/'
        ref_path='//home/junyu/project/binder_target/GP120/2ny3_b/preprocessed/reference.pkl'
        # 检查文件是否存在
        if os.path.exists(ref_path):
            # 如果文件存在，执行的操作
            print("文件存在。")
        ...
        for i in range(len(PdbDataset)):#PdbDataset:
            ...
            design_index,_=calculate_interface_residues_vtarget(ref_data['atoms14'][0],ref_data['com_idx'][0],fixed_com_idx=fixed_com_idx,n=Length)
            ...
            if hotspot :
                ...
                noisy_batch = self.Proflow.corrupt_batch_binder(new_batch, 'fp32', binder_mask,design_num,t=0,design=False ,path=design_path  )
...
def main(cfg):
    ...
    folder_path = os.path.join('/home/junyu/project/binder_target/1bj1/preprocessed', 'sampling_runs')
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
    ...
    with open(param_file, 'w') as f:
        ...
        f.write(f"Target: [1,2]\n")
        f.write(f"Length: random(50-200)\n")
        f.write(f"Hotspot Residues: []\n")
        f.write(f"Fixed by Chain: [16]\n")
        f.write(f"Number of Samples: 300\n")

    for i in range(300):
        # proflow.sample_binder_withside_randomlen(Target=2, design_num=i,min_length=15)
        # proflow.sample_binder_withside(Target=2, design_num=i)

        # length=random.randint(50,150)
        # proflow.sample_binder_bylength_reference(Target=[2], design_num=i, Length=length,fixed_com_idx=[1])

        # for short reference
        # proflow.sample_binder_bylength(Target=[2], design_num=i, Length=length)
        length = random.randint(90, 200)
        proflow.sample_binder_bylength_hotspot(Target=[1,2], design_num=i, Length=length,hotspot=['P81','P83','P91' ],fixed_by_chain=[16])  #

    # for L in [150]:
    # for i in range(100):
    #     L = random.randint(60, 200)
    #     proflow.sample_binder_reference_chains(target_com_idx=3, reference_chains=[1,5],design_num=i,Length=L)
