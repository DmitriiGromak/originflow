...
class ProteinMPNN(InverseFoldModel):
	...
	def predict(self, pdb_filepath,fix_pos_dict,):
		...
		# 遍历结构中的所有模型（通常PDB文件中只有一个模型）
		for model in structure:
			# 遍历模型中的所有链
			for chain in model:
				if chain.id not in chain_list:
					chain_list.append(chain.id)


		designed_chain_list = chain_list
		fixed_chain_list = []
		...
		folder_for_outputs = '/home/junyu/project/motif_test/base_neigh/base_rcsbcluster30_fixtopo_motif_1000_fix_update_generateall/2024-06-02_00-02-49/last/motif_sym/sym_c3_test/sym_c3_cvode_sym_tempNone_500/MPNN_outputs'
		...
		if fix_pos_dict is not None:
			fixed_positions_dict=fix_pos_dict
		else:
			fixed_positions_dict = None
		pssm_dict = None
		omit_AA_dict = None
		bias_AA_dict = None
		tied_positions_dict = 1-240,1-240,1-240
