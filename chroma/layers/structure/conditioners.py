...
class SymmetryConditioner(Conditioner):
    ...
    def symmetrize_output(self, X, C, S):
        ...
        for each in X:
          ...
          #  X_seed = each
            ...
            #save_pdb(X_tess.reshape(-1,3),'simple_copy.pdb')
    ...
    def forward(
        ...
        if self.grad_com_surgery or self.freeze_com:
            ...
            print(torch.mean(X_tess, dim=[1, 2]))
            #save_pdb(X_tess[0].reshape(-1,3),'X_Tress_scale_'+str(time.time())+'.pdb')
...
def save_pdb(xyz, pdb_out="out.pdb"):
    pdb_out='/home/junyu/project/chroma/exp/'+pdb_out
    ATOMS = ["N","CA","C","O"]
    out = open(pdb_out,"w")
    k = 0
    a = 0
