...
parser.add_argument(
    ...
    type=str,
    default='/media/junyu/DATA/bb2000pkl/')
parser.add_argument(
    '--debug',
...
def save_pdb(xyz, chain_idx, pdb_out="out.pdb"):
...
def process_mmcif(
        mmcif_path: str, max_resolution: int, max_len: int, write_dir: str, takes: str):
    ...
    # get header
    header = parsed_mmcif.get_header()
    ...
    for chain_id, chain in struct_chains.items():
        ...
        # tests = chain_dict['atom_positions'][..., [0, 1, 2, 4], :]
        # save_pdb(tests.reshape(-1, 3), chain_dict['chain_index'],pdb_out=str(xxx)+'out.pdb')
        # xxx=xxx+1
    ...
    complex_feats = du.parse_chain_feats(complex_feats)

    # # only take backbone
    if takes=='backbone':
        # complex_feats['atom_positions'] = complex_feats['atom_positions'][:, :4, :]
        complex_feats['atom_mask'] = complex_feats['atom_mask'][:, [0, 1, 2, 4]]
        complex_feats['b_factors'] = complex_feats['b_factors'][:, [0, 1, 2, 4]]
        complex_feats['atom_positions'] = complex_feats['atom_positions'][..., [0, 1, 2, 4], :]


    # tests = complex_feats['atom_positions']
    # save_pdb(tests.reshape(-1, 3), complex_feats['chain_index'],'nochuliout.pdb')
    ...
    # Radius of gyration
    metadata['radius_gyration'] = pdb_ss[0]
    complex_feats['ss'] = pdb_dg[0]
...
if __name__ == "__main__":
    process_fn('/home/junyu/project/binder_target/8x6r/8X6R.cif',max_resolution=3.,max_len=2000,write_dir='/home/junyu/project/binder_target/8x6r/')
    # Don't use GPU
