...
def chain_ids_to_sequence(chain_ids,PDB_CHAIN_IDS):
  # 假设PDB_CHAIN_IDS是已知的顺序链ID列表
  PDB_CHAIN_IDS = list(PDB_CHAIN_IDS)
  chain_id_map = {char: idx + 1 for idx, char in enumerate(PDB_CHAIN_IDS)}  # 创建从链字符到顺序索引的映射

  return np.array([chain_id_map[chain_id] if chain_id in chain_id_map else 0 for chain_id in chain_ids])
